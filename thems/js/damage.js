...
function addProductToListByID(product_id, product_name, product_code, product_price, product_part_no, product_oem_part_number, item_cost) {
    ...
    if($('#balance_'+parseInt(product_id)).length){
        bootbox.alert("Already added");
        //$('#quantity_'+parseInt(product_id)).closest('tr').find('');
        return;
    }
    ...
    var nxtCount = rowCount + 1;
    $('#rowCount').val(nxtCount);

    var product_price_dis = convertToAmount(product_price);
    var sub_total_item = convertToAmount(product_price);
    var discount_val = 0;
    var discount_val_tot = 0;
    var product_part_no_txt = product_part_no ? ', SAP Code.:' + product_part_no : '';
    var product_oem_part_number_txt = product_oem_part_number ? ', OEM Part No.:' + product_oem_part_number : '';

    var newRow = `
        <tr class="child" id="row_${nxtCount}">
            ...
            <td>
                <input type="text" class="form-control text-center rquantity" name="row[${nxtCount}][qty][]" value="${quantity}" id="quantity_${nxtCount}" onclick="this.select(); setTmpVal(this.value);" onchange="changeQtyByProductID(${product_id}, ${nxtCount});">
                 /
                <input type="text" class="form-control text-center" readonly  name="row[${nxtCount}][bal]" value="0" id="balance_${product_id}"   onchange="checkValidity(${product_id,nxtCount})">
            </td>
            <td class="text-right">
                <span class="text-right sdiscount text-danger" id="sdiscount_${nxtCount}">
                    <input type="text" name="row[${nxtCount}][discount][]" value="0" id="discount_${nxtCount}" onclick="this.select(); setTmpVal(this.value);" onchange="changeDiscountByProductID(this.value, ${nxtCount});" style="width:50px;">
                </span>
                <input type="hidden" name="row[${nxtCount}][discount_val][]" value="${discount_val}" id="discount_val_${nxtCount}">
                <input type="hidden" name="row[${nxtCount}][discount_val_tot][]" value="${discount_val_tot}" id="discount_val_tot_${nxtCount}">
            </td>
            <td class="text-right">
            ...
            <td>
                <a onclick="deleteSalesItem(${nxtCount})"><i class="fa fa-times tip podel" id="${nxtCount}" title="Remove" style="cursor:pointer;"></i></a>
            </td>
        </tr>
    `;

    $('#soTable tbody').prepend(newRow);
    calculateTotal();
    gs(parseInt(product_id),nxtCount);
}
