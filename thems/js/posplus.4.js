...
$(document).on("click", ".category", function() {}).on("click", ".subcategory", function() {
        ...
        if (sub_cat_id != $(this).val()) {
            ...
            $.ajax({
                type: "get",
                url: site.base_url + "pos/ajaxproducts",
                data: {
                    category_id: cat_id,
...
function validate_item(item) {
        ...
        for (var i = 0; i < priceKeys.length; i++) {
            ...
            if (pti.pt_id == price_type_id) {
                ...
                if(Object.keys(pti.amount).length > 1){
                    ...
                    $.each(pti.amount,(c,d)=>{
                        console.log(d);
                        prd_btn += '<button type="button" class="price-square product_validated" product_price="'+d+'" title="'+item.product_id+'" product_id="'+item.product_id+'" product_name="'+item.product_name+'">'+d+'</button>';
                    });
    ...
    return;
    if(typeof prices[0]['pt_'+price_type_id] !== "undefined"){
        var prices = prices[0]['pt_'+price_type_id];
        if(Object.keys(prices).length > 1){
            ...
            var product = {
                ...
                "product_price": c.product_price['amount_1'],
            };
            console.log(product);
            add_to_list(product);
...
function add_to_list(e) {
    ...
    var c = uuidv4().replace(/-/g, ''); //$("#count").val();
    var f;
    var a = $('<tr id="row_' + c + '" data-row-id="' + c + '"></tr>');
    f = '<td>' +
        ...
        '</td>';
    f += '<td class="text-right"><input class="form-control input-sm text-center rprice" style="width:100%;" type="text" type="text" value="' + e.product_price + '" id="pro_price_' + c + '" name="net_price[]" onClick="this.select()" ></td>"'; /* ondblclick="enable_(this)" onclick="this.select" onblur="disable_(this)" readonly*/
    ...
    f += '<td class="text-right">' +
        '<input type="hidden" value="1" id="separate_status_' + c + '" name="separate_status[]" class="pull-left"> ' +
        '<input type="hidden" value="' + e.product_price + '" id="pro_subtotal_' + c + '" name="ssubtotal[]">' +
...
function grand_total_cal(c) {
    ...
    var sub_total = 0;
    $('input[name="ssubtotal[]"]').each(function(a,b){
   ...
   /* var tot_discount = $("span#tds").text();
    if (parseInt(tot_discount) > 0) {
        ...
        if (tot_discount.indexOf("%") !== -1) {
            ...
            if (!isNaN(e[0])) {
                discount = formatDecimal((ssubtotal * parseFloat(e[0])) / 100)
            } else {
    ...
    /*$(".td_sub_total").text(formatMoney(sub_total));*/
    ...
    console.log('amount_val_1:'+(ssubtotal - discount));
    ...
    $("span#gtotal").text(formatMoney(g_total)); /*console.log('gtotal:'+(formatMoney(ssubtotal - discount + parseInt(posshipping))));*/
