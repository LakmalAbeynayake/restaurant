...
function loadItems() {
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function() {
            ...
            var product_id               = item.row.item_id;
            var item_name                = item.row.item_name;
            var product_code             = item.row.item_code;
            var product_price            = item.row.item_price_1;
            var product_cost             = item.row.item_price_1;
		    var product_oem_part_number  = item.row.item_code;
			var product_part_no          = item.row.product_part_no;
            ...
            if (ds.indexOf("%") !== -1) {
                var pds = ds.split("%");
                if (!isNaN(pds[0])) {
                    item_discount = formatDecimal(parseFloat(((product_price) * parseFloat(pds[0])) / 100));
                } else {
            ...
            unit_cost         = formatDecimal(product_price - item_discount);
            product_price     = formatDecimal(product_price);

           var newTr = $('<tr id="row_'+row_no+'" data-item-id="'+data_item_id+'"></tr>');
       //<i id="'+row_no+'" data-item="'+data_item_id+'" class="pull-right fa fa-edit tip pointer edit" title="Edit" style="cursor:pointer;"></i>     
			tr_html = '<td><span id="name_'+row_no+'" class="sname">'+item_name+'('+product_code+')  STOCK : '+ stock+'</span><input type="hidden" value="'+product_code+'" class="rcode" name="product[]"><input type="hidden" value="'+product_id+'" name="product_id[]"><input type="hidden" value="'+item_name+'" class="rname" name="product_name[]"></td>';
            
			tr_html += '<td class="text-right"><input id="cost_'+row_no+'" type="hidden" value="'+product_price+'" name="unit_cost[]" class="rucost"><span id="scost_'+row_no+'" class="text-right scost">'+accounting.formatMoney( parseFloat(product_price),"");+'</span></td>';
