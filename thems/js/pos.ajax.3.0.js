...
function widthFunctions(d) {
    ...
    $(".item-list").css("height", b - 112);
    $(".item-list").css("min-height", 534);
	$(".item-list").css("margin-right", -117);
    $("#left-middle").css("height", b - c - a - 150);
    $("#left-middle").css("min-height", 200);
    $("#product-list").css("height", b - c - a - 149);
...
$(document)/*.on("click", ".category", function() {
    ...
    if ($("#posTable >tbody > tr").length > 0) {
        ...
        if (a == 3) {
            if (shipping_address == "") {
                $("#pshipping").click();
                bootbox.alert("Please confirm and update delivery details !!")
        ...
        bootbox.alert("ADD PRODUCTS FIRST !! ");
        $("#add_item").focus()
    }
...
console.log(e[0].editable);
    ...
    //f += '<td style="background-color:orange" class="text-center posdel" onClick="delRow(' + c + ')"><i class="fa fa-times tip pointer " id="1451881153671" title="Remove" style="cursor:pointer;"></i></td>';
    f += '<td class="text-center text-muted" style="cursor:not-allowed;"><i class="fa fa-times"></i></td>';
	a.html(f);
    a.prependTo("#posTable");
