        ...
        if (a == 3) {
            ...
            if (shipping_address == "") {
                ...
                bootbox.confirm("Add order ?", function(result){ 
					 if(result == 'true'){
							form_submit_for_kot(result);
							displayNotice("", "Successfully Added ");
					}
				});
            }
        } else if (a == 1){
...
function loadItemsDef(e) { 
    ...
    f += '<td style="background-color:orange; visibility:hidden" class="text-center posdel" onClick="delRow(' + c + ')"><i class="fa fa-times tip pointer " id="1451881153671" title="Remove" style="cursor:pointer;"></i></td>';
    a.html(f);
    a.prependTo("#posTable");
    var printed = 0;
    if (typeof e[0].printed !== 'undefined') {
        printed = 1;
        console.log(e[0].printed);
    }
