    ...
    $("#conirm").click(function () {
        ...
        if (page == 'sale_add') {
            if (popup_type == 'delete') {
                var tmp = '#row_' + row_id;
                $(tmp).remove();
                displayNotice('page', 'chef name has been deleted successfully!');
                calculateTotal();
            }
        } //end page check
...
function deleteSalesItem(row_id) {
    ...
    $('#page').val('sale_add');
    $("#label").text("Are you sure you want to delete this chef name ?");
}
...
function addProductToListByID(chef_id, chef_Fname) {
    //alert(product_id);
    ...
    if (error) {
       ...
       // if (product_part_no)
           // product_part_no_txt = ', SAP Code.:' + product_part_no;
        //if(product_oem_part_number) product_oem_part_number_txt=', OEM Part No.:'+product_oem_part_number;
        //alert(rowCount);
        $('#soTable tbody').prepend('<tr class="child" id="row_' + nxtCount + '"><td>' + chef_Fname + ' (' + chef_id + ') <input name="row[' + nxtCount + '][chef_id][]" type="hidden" id="row[' + nxtCount + '][chef_id][]" value="' + chef_id + '"></td><td class="text-right"><span class="text-right sdiscount text-danger" id="sdiscount_1446800197032"><input type="sale_datetime" name="sale_datetime_on" id="sale_datetime_on" value="";?> </span> </td></td><td class="text-right"><span class="text-right sdiscount text-danger" id="sdiscount_1446800197032"><input type="sale_datetime" name="sale_datetime_off" id="sale_datetime_off" value="";?> </span></td></span>[gross_t&nbsp;</td><td><a onclick="deleteSalesItem(' + nxtCount + ')"><i class="fa fa-times tip podel" id="1446800197032" title="Remove" style="cursor:pointer;"></i></a></td></tr>');
...
$('#sale_datetime_on').datetimepicker({
defaultDate: new Date()
});
$('#sale_datetime_off').datetimepicker({
defaultDate: new Date()
});
        //  });	
        calculateTotal();
...
function changeDiscountByProductID(discount, nxtCount, product_part_no ) {
    calculateTotal();
}
