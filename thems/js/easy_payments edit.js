...
function calculateTotal(){
	...
	for(i=1; i<=rowCount; i++){
		...
		sale_inv_discount = parseInt(sale_inv_discount);
...
//alert(subtotal);
	...
	/*$.get(base_url+"sales/get_old_credit_amount?cus_id="+customer_id+"&warehouse_id=1")
	  .done(function( data ) {
		 //  window.alert(data);
		 // var obj = jQuery.parseJSON(data);
...
var sale_inv_discount=($('#sale_inv_discount').val()); sale_inv_discount = parseInt(sale_inv_discount);
...
var bill_total=totalamount_hidden;
...
{
var balance_amount = bill_total-down_payment-sale_inv_discount;
...
document.getElementById("rental_amount").value = accounting.formatMoney(rental_amount, "", 2, ",", ".");
...
document.getElementById("interest").value = accounting.formatMoney(interest_amount,"",2,"'",".");
}
