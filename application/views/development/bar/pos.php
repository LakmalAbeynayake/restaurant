...
<script type="text/javascript" src="<?php echo asset_url(); ?>js/ui-modals.js"></script>-->
    ...
    <script type="text/javascript" src="<?php echo asset_url(); ?>js/bar.ajax.3.2.js"></script>
    <script>
        ...
        function form_submit() {
            ...
            $.post("<?php echo base_url(); ?>pos/pos_submit", fields).done(function(data) {
                ...
                if (obj.error == 0) {
                    $('#fucking_done').val(1);
                    $("#soTable > tbody").empty();
                    fbs_click_pos_no_c(obj.sale_id);
        ...
        function check_kitchen_printables(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //setTimeout(check_juice_printables(id), 1000);
        ...
        function check_juice_printables(id) {
            $.ajax({
                url: "<?php echo base_url() ?>pos/check_juice_printables",
                dataType: "JSON",
                method: "POST",
                data: {
                    "cat_id": 1,
                ...
                beforeSend: function() {
                    //$('#console').text("Fetching...");  
                },
                success: function(data) {
                    if (data.success) {
                        console.log("check_juice_printables data found");
                        console.log(data);
                        $('#console').text("Data fetched!");
                        var sale_id = data.sale_id;
                        print_off_bot(sale_id);
                    } else {
                        console.log("No check_juice_printables data found");
                    }
                    setTimeout(form_reset(), 1000);
                },
                error: function(data) {
        ...
        function print_on_kot(sale_id) {
            //alert(sale_id);return false;
            <?php
            $key = "DQ2V2KR0G-d7a18a2de2ded8c1f33dbe6b1a83f5f115b04249e9d63b274cf8ffe11f6f4ded";
            $protocol = "http://";
            $domain = "isurugm.newviableerp.com/";
            $path = "pos/ap_kot_kitchen?sale_id="; //
            $dataType = "text/html";
            $printerName = "XP-80C-2"; //change here to "POS-80"
            $print_href_open = 'skyprint:?key=' . $key . '&protocol=' . $protocol . '&domain=' . $domain . '&path=' . $path;
            $print_href_close = '&dataType=' . $dataType . '&printerName=' . $printerName . '&print_id=ktchn_kot';
            ?>
            window.location.href = "<?php echo $print_href_open; ?>" + sale_id + "<?php echo $print_href_close; ?>";
        }

        function print_on_bot(sale_id) {
            <?php
            $key = "DQ2V2KR0G-d7a18a2de2ded8c1f33dbe6b1a83f5f115b04249e9d63b274cf8ffe11f6f4ded";
            $protocol = "http://";
            $domain = "isurugm.newviableerp.com/";
            $path = "pos/ap_kot?sale_id="; //
            $dataType = "text/html";
            $printerName = "XP-80C-2"; //change here to "POS-80 - kitchen"
            $print_href_open = 'skyprint:?key=' . $key . '&protocol=' . $protocol . '&domain=' . $domain . '&path=' . $path;
            $print_href_close = '&dataType=' . $dataType . '&printerName=' . $printerName . '&print_id=ktchn_kot';
