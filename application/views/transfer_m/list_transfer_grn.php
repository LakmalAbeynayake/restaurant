...
function loadGrid() {
    $('#warehouse_table').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "https://singhe-production2.vpos.verp.sites.lk/stock_transfer/get_transfer_list_with_code?code=PRODUCTION01",
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result == true) {
                jQuery.ajax({
                    type: "POST", 
                    url: "<?php echo base_url().'synchronization/grn_transfer?id='?>" + id,
                           data: 'id='+id,
                    cache: false,
