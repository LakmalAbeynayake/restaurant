    ...
    <div class="row">
      <div class="col-sm-12"> 
        ...
        <ol class="breadcrumb">
          ...
          <li> <a href="#"> TRANFER</a> </li>
          <li class="active"> ADD NEW TRANSFER ITEM  (RAW MATERIALS) </li>
          <li class="search-box">
        ...
        <div class="page-header">
          <h4> ADD NEW TRANSFER ITEM  (RAW MATERIALS) </h4>
          </div>
...
<div class="col-xs-10">
<h4 class="">TRANSFER NO : <?php echo $details['stm_no'];?></h4>
      ...
      <div class="col-md-12"> 
        ...
        <div class="panel-body">
          ...
          <div class="col-md-12">
            ...
            <!-------------------- -->
            <div class="panel panel-default">
              <div style="font-weight: 700;" class="panel-heading"> SELECT TRANFER ITEM </div>
              <div class="panel-body"> 
		...
		function loadGrid() {
		    var id=$('#odr_id').val();
					    $('#warehouse_table_1').DataTable({
					        "ajax": "<?php echo base_url('stock_m_transfer/get_trasferr_item_list?id=')?>"+id,
					        "data": {
							   id: id,
							},
					        oLanguage: {
...
$(".js-data-product-sn-ajax").select2({
        ajax: {
			'type': 'POST',
				'url': '<?php echo base_url('stock_m_transfer/get_product_dynamic');?>',
                dataType: 'json',
                delay: 0,
                data: function (query) {
                if (!query)
        ...
        $('#form_bulk_items').on('submit', function (e) {
          ...
          $.ajax({
            type: 'post',
            url: '<?php echo base_url();?>stock_m_transfer/save_transfer_item',
            data: $('form').serialize(),
  ...
  function update_product_price(id,qty){
    ...
    callback: function (result) {
        ...
        jQuery.ajax({
		type: "POST",
		url: "<?php echo base_url().'stock_m_transfer/delete_tranfer_item?id='?>"+id,
		cache: false,
		data: {id:id},
		success: function(response)
       {
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result == true) {
                jQuery.ajax({
                    type: "POST", 
                    url: "<?php echo base_url().'stock_m_transfer/final_transfer?id='?>" + id,
                           data: 'id='+id,
                    cache: false,
                    success: function(response) {
                        var obj = JSON.parse(response);
                        if (obj.status == 1) {
                            window.location.replace("<?php echo base_url().'stock_m_transfer/list_transfers'?>");
