...
<!--Start Credit-->
<div class="col-sm-5 pull-left" style="display:none">		
 <h4 class="">Credit</h4>	
 <?php
 $tot_cash_credit=0;
 ?>   
...
<table class="table table-bordered table-condensed table-hover table-striped dataTable" id="fixed_assets_table">
  <tr>
    <th width="%" class="text-center">Cashier</th>
    <th width="%" class="text-center"> Cash in Hand
   ...
   jQuery(document).ready(function () {
	...
	function search_cash_book(){
	 ...
	 /* expencess Payment Start */
	  <?php
	  $user_list = $this->User_model->getUsers();
	  // print_r($mstr_expences_type_list);
	    foreach ($user_list as $row){
			//$etp_id=$row->cat_id;
			if($row->group_id==3)
			{
	    ?>
	 $.post( "<?php echo base_url();?>finance/cash_book_get_cash_user_payments?srh_from_date="+srh_from_date+"&srh_to_date="+srh_to_date+'&user_id=<?php echo $row->user_id;?>', fields)
	.done(function( data ) {
		  var obj = jQuery.parseJSON(data);
			if(obj.error==1){
			}
			if(obj.error==0){
				...
				$("#cat_user_<?php echo $row->user_id;?>").html(obj.cash_sales_payment);
