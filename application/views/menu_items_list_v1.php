	...
	<body>
						...
						<div class="col-md-12">
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									...
									Menu
                                    <?php if ($this->session->userdata('ss_group_id')==2){ ?> 
		...
		<div class="footer clearfix">
			<div class="footer-inner">
				<?php echo date("Y"); ?> © stock management system 
			</div>
...
/*	function deleteMenuData(user_id){
	$("#myModal4").modal();
	$('#sel_id').val(user_id); 
	$('#popup_type').val('delete');
	$('#page').val('supp');
	$("#label").text("Are you sure you want to delete this menu_item?");
}*/

function disableMenuData(item_id){
	$("#myModal4").modal();
	$('#sel_id').val(item_id); 
	$('#popup_type').val('disable');
	$('#page').val('supp');
	$("#label").text("Are you sure you want to disable this menu?");
}

function enableMenuData(item_id){
	
	$("#myModal4").modal();
	$('#sel_id').val(item_id); 
	$('#page').val('supp');
	$('#popup_type').val('enable');
	$("#label").text("Are you sure you want to enable this menu_item?");
}
...
if(page=='supp'){
	if(popup_type=='delete'){
			$.post( "<?php echo base_url("menu_item/delete_menu_item"); ?>", {item_id:item_id})
		  .done(function( data ) {
		  var obj = jQuery.parseJSON(data);
				loadGrid();// load supplier data
				displayNotice('page','Menu has been deleted successfully!')
		  });
	}else if(popup_type=='disable') {
		$.post( "<?php echo base_url("menu_item/disable_menu_item"); ?>", {item_id:item_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Menu has been disabled successfully!')
	   });
	}
	else if(popup_type=='enable') {
		$.post( "<?php echo base_url("menu_item/enable_menu_item"); ?>", {item_id:item_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Menu has been enabled successfully!')
	   });
	}
} //end page check
