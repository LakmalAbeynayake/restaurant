      ...
      <div class="row">
        <div class="col-md-12"> 
          ...
          <div class="panel panel-default">
            <div class="panel-heading"> <i class="fa fa-external-link-square"></i> Vehicles
              <div class="panel-tools" style="top:2px;">
                ...
                <ul class="dropdown-menu dropdown-light pull-right">
                  <li> <a id="modal_ajax_vehicle_btn" data-toggle="modal" href="#"> <i class="fa fa-plus"></i> Add vehicle </a> </li>
                </ul>
              </div>
            </div>
          </div>
...
<script>
			...
			function loadGrid() {
					    ...
					    $('#warehouse_table').DataTable({
					        "ajax": "<?php echo base_url('vehicles/list_vehicles') ?>",
					        "bDestroy": true,
					        "iDisplayLength": 10,
							 "order": [[ 0, "desc" ]]
					    });
...
if(page=='supp'){
	if(popup_type=='delete'){
			$.post( "vehicles/delete_customer", {veh_id:veh_id})
		  .done(function( data ) {
		  var obj = jQuery.parseJSON(data);
				loadGrid();// load vehicles data
				displayNotice('page','Vehicle has been deleted successfully!')
		  });
	}else if(popup_type=='disable') {
		$.post( "vehicles/disable_customer", {veh_id:veh_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load vehicles data
			 displayNotice('page','Vehicle has been disabled successfully!')
	   });
	}
	else if(popup_type=='enable') {
		$.post( "vehicles/enable_customer", {veh_id:veh_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load vehicles data
			 displayNotice('page','Vehicle has been enabled successfully!')
	   });
	}
} //end page check
...
function deleteCustomerData(veh_id){
	$("#myModal4").modal();
	$('#sel_id').val(veh_id); 
	$('#popup_type').val('delete');
	$('#page').val('supp');
	$("#label").text("Are you sure you want to delete this vehicle?");
}

function disableCustomerData(veh_id){
	$("#myModal4").modal();
	$('#sel_id').val(veh_id); 
	$('#popup_type').val('disable');
	$('#page').val('supp');
	$("#label").text("Are you sure you want to disable this vehicle?");
}

function enableCustomerData(veh_id){
	$("#myModal4").modal();
	$('#sel_id').val(veh_id); 
	$('#page').val('supp');
	$('#popup_type').val('enable');
	$("#label").text("Are you sure you want to enable this vehicle?");
}

function click_customer_update_btn(veh_id){		
	var $modal = $('#ajax-modal');
	 $('body').modalmanager('loading');
			setTimeout(function () {
				$modal.load('<?php echo base_url("vehicles/create_customers?veh_id="); ?>'+veh_id, '', function () {
					$modal.modal();
					$("#country_id").select2();
