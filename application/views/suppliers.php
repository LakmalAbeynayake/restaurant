								...
								<div class="panel-heading">
                                    ...
                                    <div class="panel-tools" style="top:2px;">
												...
												<ul class="dropdown-menu dropdown-light pull-right">
														<li>
															<a id="modal_ajax_suppliers_btn" data-toggle="modal" href="#">
        ...
        <script>
			...
			function loadGrid() {
					    ...
					    $('#warehouse_table').DataTable({
					        "ajax": "<?php echo base_url('suppliers/list_supplier') ?>",
					        "bDestroy": true,
					        "iDisplayLength": 10,
							 "order": [[ 0, "desc" ]]
					    });
...
if(page=='supp'){
	if(popup_type=='delete'){
			$.post( "suppliers/delete", {supp_id:supp_id})
		  .done(function( data ) {
		  var obj = jQuery.parseJSON(data);
				loadGrid();// load supplier data
				displayNotice('page','Supplier has been deleted successfully!')
		  });
	}else if(popup_type=='disable') {
		$.post( "suppliers/disable", {supp_id:supp_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Supplier has been disabled successfully!')
	   });
	}
	else if(popup_type=='enable') {
		$.post( "suppliers/enable", {supp_id:supp_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Supplier has been enabled successfully!')
	   });
	}
} //end page check
...
function deleteSupplierData(supp_id){
	$("#myModal4").modal();
	$('#sel_id').val(supp_id); 
...
function disableSupplierData(supp_id){
	$("#myModal4").modal();
	$('#sel_id').val(supp_id); 
...
function enableSupplierData(supp_id){
	$("#myModal4").modal();
	$('#sel_id').val(supp_id); 
	$('#page').val('supp');
...
function click_supplier_update_btn(supp_id){		
	var $modal = $('#ajax-modal');
	 $('body').modalmanager('loading');
			setTimeout(function () {
				$modal.load('<?php echo base_url("suppliers/create_supplier?supp_id="); ?>'+supp_id, '', function () {
					$modal.modal();
