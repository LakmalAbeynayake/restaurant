...
function loadGrid() {
    $('#warehouse_table').DataTable({
        "processing": true,
        "serverSide": true,
        //"ajax": "https://singhe-admin.vpos.verp.sites.lk/index.php/api/get_api_wharehouse_transfers?w_id=6",
        "ajax": "https://singhe-store.vpos.verp.sites.lk/stock_transfer/get_stock_transfer_list/?code=4&type=Yes",
        "bDestroy": true,
        "iDisplayLength": 10,
        "order": [
            [0, "desc"]
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result == true) {
                jQuery.ajax({
                    type: "POST", 
                    url: "<?php echo base_url().'apitrns/m_grn_transfer?id='?>" + id,
                           data: 'id='+id,
                    cache: false,
...
function view_transfer(id) {
    window.location.replace("<?php echo base_url().'apitrns/view/'?>"+id);
}
