...
function check_kot(){
    jQuery.ajax({
    	type: "POST",
    	url: "<?php echo base_url().'sales/check_kot?location_id='.$location_id ?>",
    	cache: false,
    	dataType: 'json',
    	success: function(response)
    	{
...
function print_kot_auto() {
	u = location.href;
	t = document.title;
	window.open('<?php echo base_url() ?>sales/print_kot_auto?location_id='+<?php echo $location_id?>, 'sharer', 'toolbar=0,status=0,width=626,height=700, left=10, top=10,scrollbars=yes');
	return false;
}
