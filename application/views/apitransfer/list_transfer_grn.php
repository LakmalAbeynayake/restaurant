      ...
      <div class="row">
        <div class="col-md-12"> 
          ...
          <div class="panel panel-default">
            <div class="panel-heading"> <i class="fa fa-external-link-square"></i> LIST TRANSFER FOR GRN
...
function loadGrid() {
    $('#warehouse_table').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "https://singhe-admin.vpos.verp.sites.lk/index.php/api/get_api_wharehouse_transfers?w_id=4",
        //"ajax": "https://apiadmin.isurufc.newviableerp.com/index.php/api/stock_transfer/get_remote_transfer_list_with_code?w_id=1",
        "bDestroy": true,
        "iDisplayLength": 10,
        "order": [
            [0, "desc"]
...
function m_grn_this_transfer(id) {
    bootbox.confirm({
        title: "Are You Sure ?",
        ...
        callback: function(result) {
            if (result == true) {
                jQuery.ajax({
                    type: "POST", 
                    url: "<?php echo base_url().'apitransfer/m_grn_transfer?id='?>" + id,
                           data: 'id='+id,
                    cache: false,
...
function view_transfer(id) {
    window.location.replace("<?php echo base_url().'apitransfer/view/'?>"+id);
}

function view_m_transfer(id) {
    window.location.replace("<?php echo base_url().'apitransfer/m_view/'?>"+id);
}
