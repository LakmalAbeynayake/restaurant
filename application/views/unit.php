						...
						<div class="col-md-12">
							...
							<div class="panel panel-default">
								<div class="panel-heading">
                                    ...
                                    <div class="panel-tools" style="top:2px;">
												...
												<ul class="dropdown-menu dropdown-light pull-right">
														<li>
															<a id="modal_ajax_unit_btn" data-toggle="modal" href="#">
							  ...
							  <div class="panel-body">
								  ...
								  <table class="table table-bordered table-condensed table-hover table-striped dataTable" id="unit_table">
									  <thead>
							              <tr>
        ...
        <script>
			...
			function loadGrid() {
					    ...
					    $('#unit_table').DataTable({
					        "ajax": "<?php echo base_url('unit/list_unit') ?>",
					        "bDestroy": true,
					        "iDisplayLength": 10,
							 "order": [[ 0, "desc" ]]
					    });
...
if(page=='unit'){
	if(popup_type=='delete'){
			$.post( "unit/delete", {unit_id:unit_id})
		  .done(function( data ) {
		  var obj = jQuery.parseJSON(data);
				loadGrid();// load supplier data
				displayNotice('page','Unit has been deleted successfully!')
		  });
	}else if(popup_type=='disable') {
		$.post( "unit/disable", {unit_id:unit_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Unit has been disabled successfully!')
	   });
	}
	else if(popup_type=='enable') {
		$.post( "unit/enable", {unit_id:unit_id})
		     .done(function( data ) {
		     var obj = jQuery.parseJSON(data);
			 loadGrid();// load supplier data
			 displayNotice('page','Unit has been enabled successfully!')
	   });
	}
} //end page check
});
...
function deleteUnitData(unit_id){
	$("#myModal4").modal();
	$('#sel_id').val(unit_id); 
	$('#popup_type').val('delete');
	$('#page').val('unit');
	$("#label").text("Are you sure you want to delete this unit?");
}

function disableUnitData(unit_id){
	$("#myModal4").modal();
	$('#sel_id').val(unit_id); 
	$('#popup_type').val('disable');
	$('#page').val('unit');
	$("#label").text("Are you sure you want to disable this unit?");
}

function enableUnitData(unit_id){
	$("#myModal4").modal();
	$('#sel_id').val(unit_id); 
	$('#page').val('unit');
	$('#popup_type').val('enable');
	$("#label").text("Are you sure you want to enable this unit?");
}

	function click_unit_update_btn(unit_id){
		
		var $modal = $('#ajax-modal');
		 $('body').modalmanager('loading');
		 
                setTimeout(function () {
                    $modal.load('<?php echo base_url("unit/add_unit?unit_id="); ?>'+unit_id, '', function () {
                        $modal.modal();
                    });
                }, 1000);
	}

            $('a#modal_ajax_unit_btn').on('click', function () {
                // create the backdrop and wait for next modal to be triggered
                $('body').modalmanager('loading');
                setTimeout(function () {
                    $modal.load('<?php echo base_url("unit/add_unit"); ?>', '', function () {
                        $modal.modal();
                    });
                }, 1000);
            });
