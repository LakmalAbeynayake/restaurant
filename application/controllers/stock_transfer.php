...
class Stock_Transfer extends CI_Controller
{
    ...
    function new_transfer(){
		...
		$data['outlet_list']=$this->Stock_Transfer_Model->get_outlet_list();
		
		$this->load->view('transfer/add_transfer_master',$data); 
	}
	...
	//	$this->form_validation->set_rules('odr_price_type_id', 'PRICE TYPE', 'required|xss_clean');
        ...
        $data=array(
			...
			'stm_note'=>$note,	
			'stm_to_id'=>$odr_type,
			'stm_no'=>$this->Common_Model->gen_ref_number('stm_id','stock_transfer_master','TEO'),
			'stm_by'=>$this->session->userdata('ss_user_id'),
			'stm_date_time'=>date("Y-m-d H:i:s"),
			
		);	
          $result = $this->Stock_Transfer_Model->save_trasfer_master($data);
        $status=0;
        if($result>0){
           $status=1; 
        }
	...
	function add_transfer_items(){
		...
		$data['details']=$this->Stock_Transfer_Model->get_transfer_info($data['stm_id']);
	    $this->load->view('transfer/add_transfer_items',$data);
	}
	
	public function get_product_dynamic(){
        $str = $this->input->post('search_string');
		$result=$this->Stock_Transfer_Model->get_product_dynamic($str);
		$data=array();
		foreach($result as $r){
		    $stock=$this->Stock_Counter_Model->get_outlet_finish_product_stock_count($r['product_id']);
		    ...
		    $nested_data=array(
		        'batch_id'=>$r['product_id'],
		        'product_id'=>$r['product_id'],
		        'product_name'=>$r['product_name'],
		        'product_code'=>$r['product_code'],
		        'batch'=>",   ".$r['product_code'].", COST RS.".$r['product_cost'].", STOCK ".$stock,
		        );
		        $data[]=$nested_data;
		}
        ...
        $str = $this->input->post('search_string');
		$result=$this->Stock_Transfer_Model->get_product_value_added_dynamic($str);
		$data=array();
		foreach($result as $r){
	...
	public function save_transfer_item(){
        ...
        else
        {
	    	...
	    	$price_cost=$this->Stock_Transfer_Model->get_product_price_cost($product_id);
	    	
	    	$stock=$this->Stock_Counter_Model->get_outlet_finish_product_stock_count($product_id);
	    	if($stock < $req_qty){
			...
			'product_retail_pirce'=> $price_cost['product_price'],
			'product_wholsale_pirce'=>$price_cost['product_price'],
			'product_credit_pirce'=>$price_cost['product_price'],	
			'product_code'=>$price_cost['product_code'],
			'batch_id'=>$product_id,
		);	
         $result=$this->Stock_Transfer_Model->save_trasfer_item($data);	   
        ...
        $values = $this->Stock_Transfer_Model->get_trasfer_product_list($id);
    ...
    function delete_tranfer_item(){
        ...
        else
        {
            $id=$this->input->post('id');
             $result=$this->Stock_Transfer_Model->delete_trasfer_item($id);	   
          $st = array('status' =>1,'validation' =>'Added');
          echo json_encode($st);
		}
    ...
    function final_transfer(){
		...
		$result=$this->Stock_Transfer_Model->update_transfer_master($id,$data);
		$list=$this->Stock_Transfer_Model->get_transfer_list_items($id);
		foreach($list as $row){
		     $data=array(
		    ...
		    //$this->Stock_Transfer_Model->save_stock_lodge($data);
    ...
    function list_transfers(){
		...
		$data['sub_menu_name'] = 'list_transfers';
	    $this->load->view('transfer/list_transfer',$data);
	}
	...
	public function get_transfer_list($value = ''){
        ...
        $totalData      = 0;
        $values         = $this->Stock_Transfer_Model->get_transfer_list($start, $length, $search_key_val);
        $value_count    = $this->Stock_Transfer_Model->get_transfer_list('', '', '');
        if ($search_key_val) {
            $values_c  = $this->Stock_Transfer_Model->get_transfer_list('', '', $search_key_val);
            $totalData = 100;  
        }
        ...
        if (!empty($values)) {
            foreach ($values as $products) {
                ...
                $option_order_details='<li><a href="' . base_url('stock_transfer/transfer_details') . '?id=' . $products->stm_id . '"><i class="fa fa-file-o"></i> DETAILS</a></li>';
                $update_action='<li><a href="' . base_url('stock_transfer/add_transfer_items') . '?id=' . $products->stm_id . '"><i class="fa fa-pencil-square-o"></i> UPDATE</a></li>';
    ...
    function transfer_details(){
		...
		$data['stm_id'] =$this->input->get('id');
		$data['details']=$this->Stock_Transfer_Model->get_transfer_info($data['stm_id']);
	    $this->load->view('transfer/transfer_details',$data);
	}
    ...
    public function get_transfer_list_with_code($value = ''){
        ...
        $length         = $this->input->get('length');
        $totalData      = 0;
        $values         = $this->Stock_Transfer_Model->get_transfer_list_with_code($code,$start, $length, $search_key_val);
        $value_count    = $this->Stock_Transfer_Model->get_transfer_list_with_code($code,'', '', '');
        if ($search_key_val) {
            $values_c  = $this->Stock_Transfer_Model->get_transfer_list_with_code($code,'', '', $search_key_val);
            $totalData = 100;  
        }
        $totalData     = $value_count;
        $totalFiltered = $totalData;
        ...
        if (!empty($values)) {
            foreach ($values as $products) {
                ...
                $option_order_details='
                 ...
                 <li><a  class="btn btn-default " data-toggle="tooltip" data-placement="bottom" title="GRN This Transfer"  onClick="view_transfer('.$products->stm_id.')"><i class="fa fa-check" aria-hidden="true"> Details</i></a></li> 
                <li><a  class="btn btn-default " data-toggle="tooltip" data-placement="bottom" title="GRN This Transfer"  onClick="grn_this_transfer('.$products->stm_id.')"><i class="fa fa-check" aria-hidden="true"> GRN</i></a></li>';
                $update_action='';
