...
class Reports extends CI_Controller {
	...
	//	echo "srh_to_date 1 $srh_to_date";
	$data=array();
...
//	echo "srh_to_date 2 $srh_to_date";
	...
	$opening_stock_qty=0;
	$opening_stock_from_date='2019-10-18';//date('2018-06-24');
...
//	echo "srh_to_date 3 $srh_to_date";
	$test_var='';
...
//	echo "sold_qty:$sold_qty";
	...
	//if($sold_val_balance<0){
	   // $sold_val_balance=-1*$sold_val_balance;
...
//	$warehouse_opening_stock_tot=$warehouse_opening_stock;
	...
	$pur_val='';
	if($pur_val) $pur_val=$purchased_qty*$product_info->product_cost;
...
//get_list_payments_for_report_grn
    ...
    public function get_list_payments_for_report() {
        ...
        if (!empty($values)) {
            foreach ($values as $users) {
                ...
                $row[] = sprintf('%04d', $users->sale_id);//$users->sale_reference_no;
    ...
    public function get_list_sales_for_print($value = '') {
        ...
        foreach ($sales as $row) {
            ...
            $nestedData[] = sprintf('%04d', $row['sale_id']);//($row['sale_id'];//$row['sale_reference_no'];
    ...
    public function get_list_sales_for_print_daily($value = '') {
        ...
        foreach ($sales as $row) {
            ...
            $total_paid_amount = $row['total_paid_amount']; //$this->Sales_Model->get_total_paid_by_sale_id($sale_id);
            $nestedData[] = display_date_time_format($row['sale_datetime']);
            $nestedData[] = sprintf('%04d', $row['sale_id']);//$row['sale_reference_no'];
...
$os_tra_in=0;
	...
	$opening_stock_date=$day_before;
	$opening_stock_qty=0;
	$opening_stock_from_date='2019-10-23';//date('2018-06-24');
	$date_1=date( 'Y-m-d', strtotime( $opening_stock_from_date . ' +1 day' ) );
	$os_i=0;//$this->Report_Model->get_location_issue_by_location_id_and_date($location_id,$product_id,$date_1,$day_before);
	//os grn
	$os_grn=0;
	...
	$os_tra_out =0;// $this->Transfer_Model->getTransferdQtyByWarehouseId($srh_warehouse_id, $products->product_id);
	...
	$os_grn=0;//$this->Purchases_Model->getPurchasedQtyByWarehouseIdAndDateRange($location_id,$product_id,$date_1,$day_before);
	
	$os_grn_rtn=0;//$this->Purchases_Model->getPurchaseRTNdQtyByWarehouseIdAndDateRange($location_id,$product_id,$date_1,$day_before);
	$os_ir=0;//$this->Report_Model->get_location_issue_return_by_location_id_and_date($location_id,$product_id,$date_1,$day_before);
	$os_s =0;// $this->Report_Model->get_location_sale_by_location_id_and_date($location_id,$product_id,$date_1,$day_before);
	//$os_s=$this->Report_Model->get_location_sale_by_location_id_and_date(0,$product_id,$srh_from_date,$srh_to_date,'',$srh_warehouse_id);
	//echo $this->db->last_query();
...
//	echo "srh_to_date 3 $srh_to_date";
	...
	$opening_stock_qty_des=0;//"<br/>wos:$warehouse_opening_stock , os_i:$os_i ,os_ir:$os_ir ,os_s:$os_s ,os_h:$os_h , os e :$os_exchange_qty:";
	//$opening_stock_qty=$os_i-$os_ir-$os_s-$os_h;
	$opening_stock_qty=0;//$warehouse_opening_stock+$os_i-$os_ir-$os_s-$os_h;
	$warehouse_opening_stock_tot=0;
...
//$row[] = number_format($product_damaged_qty, 2, '.', '');
              //  $row[] = ":(" . $each_product_cost . "*" . $purchased_qty . ")=";
                $row[] = $next_day_openning_balance;//number_format($next_day_openning_balance, 2, '.', ''); //$switch .  
               // $row[] = "(".$product_price."*".$sold_qty.")=";
                $row[] = 0;//number_format($sale_price_sub_tot, 2, '.', ''); //
