...
class Customer extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('order/pos', $data);
    }
    public function vk()
    {
        $this->load->view("order/vk");
    }
...
" . $pro->product_name . "<br>" . $pro->product_price . "
    ...
    public function view_bill()
    {
        ...
        $data['get_customer']     = $this->pos_model->get_customer();
        $data['get_warehouse']    = $this->pos_model->get_warehouse();
        $this->load->view('order/pos-bill', $data);
    }
	...
	{   
        ...
        if ($search_key_val) {
			//echo 'hello 1-';
            $sales     = $this->pos_model->get_all_sales($start, $length, $search_key_val, $dine_type, '1');
            $sales_c   = $this->pos_model->get_all_sales_c('', '', $search_key_val, $dine_type, '1');
			$sales_c   = $sales_c[0]['count_s'];
			//print_r($sales_c);
            $totalData = $sales_c;
        } else {
			//echo 'hello 2-';
            $sales     = $this->pos_model->get_all_sales($start, $length, '', $dine_type, '1');
            $sales_c   = $this->pos_model->get_all_sales_c('', '', '', $dine_type, '1');
            $sales_c   = $sales_c[0]['count_s'];
			//print_r($sales_c); 
            $totalData = $sales_c;
        }
