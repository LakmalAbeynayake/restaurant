...
class Apitrns extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('apitrns/list_transfer_grn', $data);
    }
    ...
    public function material()
    {
        $data['main_menu_name'] = $this->main_menu_name;
        $data['sub_menu_name']  = $this->sub_menu_name;
        $this->load->view('apitrns/material', $data);
    }
    ...
    public function grn_transfer()
    {
        ...
        $url ="http://singhe-store.vpos.verp.sites.lk/api/transfer_grn?id=".$id;
        $ch = curl_init($url);
        $headers = array(
        ...
        foreach($product_list as $pd){
            $pcode = $pd->product_code;
        $product_id=$this->Product_Models->get_product_id($pcode);
       if($product_id==0){$operation=0; $err_product_code.=$pcode." , "; }
      ...
      $last_id=$this->API_Model->save_grn_master($data);  
        ...
        $url ="http://singhe-store.vpos.verp.sites.lk/stock_transfer/update_grn_recode?id=".$id;
        $ch = curl_init($url);
        $headers = array(
            "Content-Type: application/json"
            );
    ...
    public function view($id)
    {
        ...
        $url ="http://singhe-store.vpos.verp.sites.lk/api/transfer_grn/?id=".$id;//change by namal
        $ch = curl_init($url);
        $headers = array(
            "Content-Type: application/json"
            );
        ...
        $this->load->view('apitrns/transfer_details', $data);
    }
