...
class Quotations_Model extends CI_Model
{
    ...
    {
        $this->db->select("q.*,s.sale_reference_no,s.sale_total,s.sale_datetime,u.user_first_name");
        $this->db->from("quotations q");
        $this->db->join("sales s", "q.qts_id = s.sale_id", "left");
        $this->db->join('user u', 'q.user_id = u.user_id', 'left');
        $this->db->where("q.qts_id", $id);
        $this->db->order_by("q.qts_id", "desc");
        $query = $this->db->get();
    ...
    {
        $this->db->select("quotations_items.product_id, product.product_name, product.product_code, quotations_items.quantity, quotations_items.discount, quotations_items.discount_val, quotations_items.unit_price, quotations_items.gross_total");
    ...
    //Sales item save
    function save_quotations_item(&$data_item,$qts_id)
    {
        $this->db->insert("quotations_items_log", $data_item);
    ...
    //Sales all get
    function get_all_quotations($location_id="",$sf=0)
    {
        $this->db->select("quotations.*, customer.cus_name");
        $this->db->from("customer");
        $this->db->join("quotations", "quotations.customer_id = customer.cus_id", "inner");
        $this->db->order_by("quotations.qts_id", "desc");
        $this->db->where("quotations.qts_id IS NOT NULL"); //("id !=",$id);
        
        if($sf == 0){
            
            $this->db->where("quotations.qutation_status","1");
        }
        
        if($location_id)
            $this->db->where("quotations.warehouse_id",$location_id);
    ...
    function get_all_products()
    {
     ...
     function update_quotations($id, $data)
    {
        if ($id>0) {
