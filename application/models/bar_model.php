...
class Bar_Model extends CI_Model
{
    ...
    function get_product_by_code($product_code = '', $customer_id = '', $warehouse_id = '')
    {
        ...
        if ($product_code) {
            $this->db->like('p.product_status = 1 AND p.product_name', $product_code);
            $this->db->or_like('p.product_status = 1 AND p.product_code', $product_code);
            $this->db->limit('5');
        }else{
             $this->db->where('product_status', 1);
        }
     ...
     function get_waiter()
    {
        ...
        $this->db->where('warehouse_id', $this->session->userdata('ss_warehouse_id'));
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
    ...
    {
        $this->db->select('id,product_id,product_name,quantity,unit_price');
