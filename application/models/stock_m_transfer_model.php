...
class Stock_M_Transfer_Model extends CI_Model {
  ...
  function __construct() 
  {
	...
	{
		 $this->db->insert('stock_m_transfer_master',$data);
		 return $this->db->insert_id();
	}
	function save_trasfer_item($data)
	{
		 $this->db->insert('stock_m_transfer_item',$data);
		 return $this->db->insert_id();
	}
	function get_list_trasfer() {
	    $this->db->select("s.*");
        $this->db->from('stock_m_transfer_master s');
	...
	function get_product_dynamic($search_key_val){
	    $this->db->select("o.*");
        $this->db->from('menu_item o');
        $this->db->where("o.item_status = 1 AND  o.item_name LIKE '$search_key_val%'");
        $this->db->or_where("o.item_status = 1 AND  o.item_code LIKE '$search_key_val%'");
        //$this->db->or_where("o.batch_status = 1 AND  o.batch_ref_number LIKE '$search_key_val%'");
        $this->db->group_by('o.item_id');
        $this->db->order_by('o.item_id','desc');
        $this->db->limit(100);
        $query = $this->db->get();
        return $query->result_array();
	}
	...
	public function get_product_price_cost($id)
	{
		...
		function get_trasfer_product_list($id){
	     $this->db->select("o.*,p.item_name,p.item_code,u.unit_code");
        $this->db->from('stock_m_transfer_item o');
        $this->db->join("menu_item p", "p.item_id = o.product_id", "inner");
        $this->db->join("mstr_unit u", "u.unit_id = p.item_unit", "inner");
        $this->db->where('o.stm_id',$id);
        $query = $this->db->get();
        return $query->result_array();
	}
	 public function delete_trasfer_item($id){
		...
		return $this->db->delete('stock_m_transfer_item'); 
	    }else{
	        return false;
	    }
	}
	...
	public function update_transfer_master($id,$data){
		...
		return $this->db->update('stock_m_transfer_master',$data);
	     }else{
	        return false;
	    }
	}
	...
	function get_trasfer_product_list_api($id){
	    $this->db->select("o.*,p.product_name,p.item_code,u.unit_code,stm.stm_no,stm.stm_receved_status");
        $this->db->from('stock_m_transfer_item o');
        $this->db->join("stock_m_transfer_master stm", "stm.stm_id = o.stm_id", "inner");
	...
	//Added by Namal 
	function get_trasfer_product_list_api_new($id){
	    $this->db->select("o.*,p.item_name,p.item_code,u.unit_code,stm.stm_no,stm.stm_receved_status");
        $this->db->from('stock_m_transfer_item o');
        $this->db->join("stock_m_transfer_master stm", "stm.stm_id = o.stm_id", "inner");
        $this->db->join("menu_item p", "p.item_id = o.product_id", "inner");
        $this->db->join("mstr_unit u", "u.unit_id = p.item_unit", "inner");
	...
	function get_transfer_list_items($id){
        ...
        $this->db->from('stock_m_transfer_item o');
        $this->db->join("stock_m_transfer_master s", "s.stm_id = o.stm_id", "inner");
        $this->db->where('o.stm_id',$id);
        $query = $this->db->get();
        return $query->result_array();
	}
	...
	function get_product_mat_id($code){
	    $this->db->select("p.product_id");
        $this->db->from('product_material p');
        $this->db->where('o.product_mat_code',$code); 
	    $query = $this->db->get();
